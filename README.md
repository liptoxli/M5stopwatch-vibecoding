...
<p align="center">
  <img alt="Firmware v0.10.2" src="https://img.shields.io/badge/firmware-v0.10.2-6f5cff">
  <img alt="Bridge v1.3.2" src="https://img.shields.io/badge/macOS%20Bridge-v1.3.2-35b8ff">
...
当前版本为 **固件 v0.10.2 / macOS Bridge v1.3.2**。语音输入、虚拟麦克风、BLE HID、原生 Codex Micro 兼容交互和圆屏触摸均已在真实设备上完成端到端验证。本次维护版修复了设备重启后偶发丢失配对、必须在 macOS 中“忽略此设备”才能恢复的问题，并完善 Bridge 的原生 HID 交接与认证等待机制。
...
当前版本适合实机体验、二次开发和日常语音输入。电量百分比仍以电池电压估算，低电量区会偏保守；因此页面中的续航数字是实测范围，不是实验室标称值。首次从 v0.9.x 升级到 v0.10.x 时，由于 HID 描述符发生变化，需要在 macOS 中忽略旧的 `M5Codex-*` 并重新配对一次；已经在 v0.10.x 正常配对的设备升级 v0.10.2 时无需再次忽略设备。
