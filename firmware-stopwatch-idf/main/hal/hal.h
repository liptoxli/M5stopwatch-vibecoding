...
public:
    ...
    void updateActivityMonitor();
    void markActivity();
    bool isActivitySleeping() const
    {
        return _activity_sleeping;
    }
    ...
    /* -------------------------------- Bluetooth ------------------------------- */
    void setBluetoothEnabled(bool enabled, bool saveToSettings = false);
    bool getBluetoothEnabled(bool loadFromSettings = false);
...
private:
    ...
    int _spk_volume    = 80;
    bool _activity_sleeping = false;
    bool _activity_dimmed = false;
    bool _activity_has_imu_baseline = false;
    bool _activity_button_a_down = false;
    bool _activity_button_b_down = false;
    bool _cpu_low_power_mode = true;
    int _activity_saved_brightness = 80;
    std::uint32_t _activity_last_ms = 0;
    std::uint32_t _activity_last_poll_ms = 0;
    ImuData _activity_imu_baseline;

    void i2c_init();
    void i2c_detect();
    void pmic_init();
    bool pmic_get_pwr_btn_state();
    void enterActivityDim();
    ...
    void displayExitActivitySleep();
    void requestFullDisplayRedraw();
    void ioe_init();
    void ioe_tp_reset();
