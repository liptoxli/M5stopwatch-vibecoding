...
public:
    ...
    static constexpr TickType_t speaker_amp_idle_ticks = pdMS_TO_TICKS(650);

    void init(i2c_master_bus_handle_t i2c_bus, bool enableInput)
    {
        _input_enabled = enableInput;
        ...
        es8311_codec_cfg_t es8311_cfg = {
            .ctrl_if     = _ctrl_if,
            .gpio_if     = _gpio_if,
            .codec_mode  = _input_enabled ? ESP_CODEC_DEV_WORK_MODE_BOTH : ESP_CODEC_DEV_WORK_MODE_DAC,
            .pa_pin      = GPIO_NUM_NC,
            .pa_reverted = false,
            .use_mclk    = true,
        };
        ...
        esp_codec_dev_cfg_t dev_cfg = {
            .dev_type = _input_enabled ? ESP_CODEC_DEV_TYPE_IN_OUT : ESP_CODEC_DEV_TYPE_OUT,
            .codec_if = _codec_if,
            .data_if  = _data_if,
        };
...
private:
        ...
        while (1) {
            ...
            while (true) {
                ...
                if (current_data.empty()) {
                    _set_speaker_amp(false);
                    break;
                }

                _set_speaker_amp(true);
                ...
                if (ulTaskNotifyTake(pdTRUE, speaker_amp_idle_ticks) > 0) {
                    continue;
                }
                _set_speaker_amp(false);
            }
    ...
    void _write(const std::vector<int16_t>& data)
    {
        ...
        vTaskDelay(speaker_amp_idle_ticks);
        _set_speaker_amp(false);
    }

    void _set_speaker_amp(bool enabled)
    {
        if (_speaker_amp_enabled == enabled) {
            return;
        }
            ...
            .gpio_cfg =
                {
                    ...
                    .din  = enableInput ? I2S_DADC_IN_PIN : GPIO_NUM_NC,
                },
        };

        ESP_ERROR_CHECK(i2s_new_channel(&chan_cfg, &_tx_handle, enableInput ? &_rx_handle : nullptr));
...
void Hal::audio_init()
{
    if (!codex_config::kEnableAudioOutput) {
        mclog::tagInfo(_tag, "disabled");
    ...
    _audio_codec.init(i2c_bus_get_internal_bus_handle(_i2c_bus), codex_config::kEnableAudioCodecInputPath);
...
int Hal::getSpeakerVolume(bool loadFromSettings)
{
    ...
    } else if (codex_config::kEnableAudioOutput) {
