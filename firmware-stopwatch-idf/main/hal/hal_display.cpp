...
public:
    ...
    void sleepPanel()
    {
        ...
        _panel_instance.setSleep(true);
    }

    void wakePanel()
    {
        _panel_instance.setSleep(false);
        _panel_instance.setPowerSave(false);
    }
};
...
void Hal::displayEnterActivitySleep()
{
    if (!_display) {
        return;
    }

    mclog::tagInfo(_tag, "display enter activity sleep");
    _display->sleepPanel();
}

void Hal::displayExitActivitySleep()
{
    if (!_display) {
        return;
    }

    mclog::tagInfo(_tag, "display exit activity sleep");
    _display->wakePanel();
    _display->display();
}
...
void Hal::lvgl_init()
{
    ...
    const size_t draw_buffer_size = _display->width() * LV_BUFFER_LINE * sizeof(lv_color_t);
    static uint8_t *buf1 = (uint8_t *)heap_caps_malloc(draw_buffer_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    static uint8_t *buf2 = (uint8_t *)heap_caps_malloc(draw_buffer_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    LV_ASSERT_MALLOC(buf1);
    LV_ASSERT_MALLOC(buf2);
    if (buf1 == NULL || buf2 == NULL) {
        printf("lv display buffer malloc failed\n");
        return;
    }
...
{
    if (!_display || xGuiSemaphore == nullptr) {
        return;
    }
